...
def make_config(scarcity: str, ncities: str) -> Path:
    TMP.mkdir(parents=True, exist_ok=True)
    cfg = yaml.safe_load((REPO / "configs" / "env_default.yaml").read_text())
    cfg["weekly_supply_per_capita"] = float(scarcity)
    cfg["n_cities"] = int(ncities)
    out = TMP / f"env_s{scarcity}_n{ncities}.yaml"
...
def build_jobs() -> list[dict]:
    ...
    points: list[tuple[str, str]] = [(s, BASE_NCITIES) for s in SCARCITIES]
    points += [(BASE_SCARCITY, n) for n in N_CITIES]
    jobs = []
    for scarcity, ncities in points:
        cfg = make_config(scarcity, ncities)
        suffix = sweep_suffix(scarcity, ncities)
        for algo in ALGOS:
            for seed in SEEDS:
                log_dir = RUNS / f"{algo}{suffix}_seed{seed}"
...
def main() -> int:
    ...
    print(f"[grid] {len(jobs)} total jobs, {len(jobs) - len(todo)} already complete, "
    ...
    subprocess.run([sys.executable, "-m", "src.eval.analyze", str(RUNS)], cwd=REPO)
    write_timings()
    return 0


def write_timings() -> None:
    """Regenerate runs/run_timings.csv from each run's train.log final timing line."""
    import re
    rows = []
